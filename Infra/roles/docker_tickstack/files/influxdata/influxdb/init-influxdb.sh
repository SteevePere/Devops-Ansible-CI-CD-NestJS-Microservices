...
if ( [ ! -z "$INIT_USERS" ] || [ ! -z "$INFLUXDB_DB" ] || [ "$(ls -A /docker-entrypoint-initdb.d 2> /dev/null)" ] ) && [ ! "$(ls -d "$META_DIR" 2>/dev/null)" ]; then
	...
	INFLUXDB_HTTP_BIND_ADDRESS=192.168.1.99:$INFLUXDB_INIT_PORT INFLUXDB_HTTP_HTTPS_ENABLED=false influxd "$@" &
	pid="$!"

	INFLUX_CMD="influx -host 192.168.1.99 -port $INFLUXDB_INIT_PORT -execute "
	...
	if [ ! -z "$INIT_USERS" ]; then
		...
		INFLUX_CMD="influx -host 192.168.1.99 -port $INFLUXDB_INIT_PORT -username ${INFLUXDB_ADMIN_USER} -password ${INFLUXDB_ADMIN_PASSWORD} -execute "
